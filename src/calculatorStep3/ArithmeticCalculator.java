...
public class ArithmeticCalculator<T extends Number> {
    ...
    public void biggerThan(double thisNum1) {
        // 계산 결과 값을 스트림에 호출합니다.
        results.stream()
    ...
    public void smallerThan(double thisNum2) {
        results.stream()
                .filter(result -> result < thisNum2)
                .forEach(result -> System.out.println(result));
    }
    // 입력받은 값과 같은 계산 결과를 출력합니다.
    public void equalTo(double thisNum3) {
        results.stream()
                .filter(result -> result == thisNum3)
                .forEach(result -> System.out.println(result));
    }
